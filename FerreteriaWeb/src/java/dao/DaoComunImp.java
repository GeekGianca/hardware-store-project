...
public class DaoComunImp {
    ...
    public List<V_CompraNested> listarVentas(){
        ...
        try {
            ...
            while (rs.next()) {
                ...
                compras.setCantidadProductos(rs.getInt("cantidad_productos"));
                compras.setTotalVenta(rs.getDouble("total_venta"));
                listar.add(compras);
            }
        } catch (SQLException ex) {
            Logger.getLogger(this.getClass().getName()).log(Level.SEVERE, null, ex);
    ...
    public List<F_CompraNested> buscar(String like){
        this.conf = FactoriaMysql.getInstanceOpenConexion(FactoriaMysql.MYSQL);
        StringBuilder sql = new StringBuilder();
        sql.append("SELECT `idfactura`, `cantidad_productos`, `total_venta`, `Proveedor_idproveedor`, `detalle_venta`,`fechahora_venta` FROM `facturas` INNER JOIN `f_compra` ON `facturas`.`idfactura` = `f_compra`.`Factura_idfactura` WHERE `fechahora_venta` LIKE '%").append(like).append("%' OR `idfactura` LIKE '%").append(like).append("%';");
        List<F_CompraNested> listar = null;
        try {
            listar = new ArrayList<>();
            ResultSet rs = this.conf.query(sql.toString());
